...
interface ExSkillData {
    ...
    intro: any;
    /** 子技能；实现一个技能实现多个效果， 结构存在争议，往后继续研究 */
...
interface ExAIData {
    order: number,
    threaten: number | ((player, target)=>number);
    expose:number;
    ...
    effect: {
        target(card, player, target, current):string;
    }

    basic:any;
    tag:any;

    result(player, target):any;
}
